...
async def test_user_create(client: AsyncClient):
    ...
    response = await client.post("/users", json=user_create.model_dump())
    data = response.json()
...
async def test_user_create_invalid_name(client: AsyncClient):
    ...
    response = await client.post("/users", json={"name": 13})
    assert response.status_code == 422


async def test_user_create_user_exists(client: AsyncClient, current_user: User):

    response = await client.post("/users", json={"name": current_user.name})
